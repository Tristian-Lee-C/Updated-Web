...
<?php
	...
	$TierMulti = $statementTier->fetchAll();
...
<html>
	<head>
		...
		<link rel="stylesheet" href="Tiers_template.css">
	...
	<body>
		...
		<div id="wrapper">
			...
			<div class='subNav'>
				...
				<div class='WebGut'>
					<div class='CreateTierSect'>
					<form class ="addTierButton" action="AddTierPU.php" method="POST" id='mainForm'><input type="submit" value="Add" name='addTier'></form>
					<div class='Scroll'>
					<table class ='Tier_list'>
							<thead>
								<tr>
									<th>Tier ID</th>
									<th>Service Tier</th>
							...
							<?php foreach ($TierMulti as $TierSingle): ?>
							<?php 
							$grabIDTier = $TierSingle['tierID'];
							$queryFilter = 'SELECT delete_fact
											FROM deleted_tiers
											WHERE tierID = :grabIDTier;';
							$statementFilter = $db->prepare($queryFilter);
							$statementFilter->bindValue(':grabIDTier', $grabIDTier);
							...
							?>
							<tbody>
								<td><?php echo $TierSingle['tierID'];?></td>
								<td><?php echo $TierSingle['tier_level'];?></td>
								<td>
									<form action="" method="post">
										<input type="hidden" name="TierID" value="<?php echo $TierSingle['tierID']; ?>">
										<label class="switch">
											<input class="TierToggle" type="checkbox" name='TierToggle' <?php if($TierSingle['on_off'] == TRUE){?> checked <?php }; ?>>
								...
								<?php if (isset($_SESSION["is_administator"])){?>
											<form class ="editTierButton" action="editTierPU.php" method="POST">
											<input type='hidden' name='TierIDEdit' value='<?php echo $TierSingle['tierID'];?>'>
											<input type='submit' value='Edit' name='editTier'>
											</form>
								<?php }; ?>
							
								<?php if (isset($_SESSION["is_administator"])){?>
											<form class ="deleteTierButton" action="deleteTierPU.php" method="POST">
											<input type='hidden' name='TierIDDelete' value='<?php echo $TierSingle['tierID'];?>'>
											<input type='submit' value='Delete' name='deleteTier'>
											</form>
								<?php }; ?></td>
							</tbody>
							<?php }; ?>
...
function checkBoxAction(event) {
    ...
    var TierID = checkbox.closest('tr').querySelector('input[name="TierID"]').value; // Get the staffID associated with the checkbox
    console.log('isChecked:', isChecked); // Debugging statement
    console.log('TierID:', TierID); // Debugging statement
    ...
    xhttp.open("GET", "updateTierDB.php?isChecked=" + isChecked + "&TierID=" + TierID, true);
    xhttp.send();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var checkboxes = document.querySelectorAll('input.TierToggle');
